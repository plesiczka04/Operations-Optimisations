...
total_aircraft_count = df['Aircraft'].nunique()
num_accepted_aircraft = df_accepted['Aircraft'].nunique()
num_rejected_aircraft = df_rejected['Aircraft'].nunique()
...
rejected_table_data = df_rejected.drop_duplicates(subset=['Aircraft'])[[
    'Aircraft', 'ETA', 'ETD', 'ServT'
]].rename(columns={'Aircraft': 'ID'})
...
def draw_static_hangar_for_export(ax, time, df_accepted_data):
    ...
    for _, ac in df_accepted_data.iterrows():
        if ac['Roll_In'] <= time < ac['Roll_Out']:
            ...
            ax.text(x + w / 2, text_y_ref + l * 0.03, f"#{ac['Aircraft']}", ha='center', va='bottom', fontsize=title_fontsize, weight='bold', zorder=11, color=COLORS["text"], stretch='semi-expanded')
...
def animate_multiple_aircrafts(acs, enters):
    ...
    for f in range(total_frames):
        ...
        for idx, ac in enumerate(acs):
            ...
            id_text = ax_hangar.text(x + w / 2, text_y_ref + l * 0.03, f"#{ac['Aircraft']}", ha='center', va='bottom', fontsize=title_fontsize, weight='bold', zorder=11, color=COLORS["text"], stretch='semi-expanded')
