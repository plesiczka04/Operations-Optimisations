...
DEFAULT_PATTERN = os.path.join(os.path.dirname(__file__), "solution_buffer_*.csv")
# If you ran main.py into a 'results' folder, you can set e.g.:
# DEFAULT_PATTERN = "results/*_buffer_*.csv"


def parse_factor_from_filename(path: str) -> float:
    """
    Extract epsilon_t-penalty factor from a filename like
    'solution_buffer_0p25.csv' or 'T3_buffer_1p0.csv'.

    It looks for the substring '_buffer_' in the stem, takes everything
    after it up to the extension, replaces 'p' by '.', and converts to float.
    """
    base = os.path.basename(path)
    stem, _ = os.path.splitext(base)
    if "_buffer_" not in stem:
        raise ValueError(
            f"Cannot extract factor from filename {base!r}: no '_buffer_' substring."
        )
    tag = stem.split("_buffer_", 1)[1]
    if not tag:
        raise ValueError(
...
def plot_sensitivity(rows: List[Dict[str, Any]], prefix: str = "sensitivity_buffer") -> None:
    """
