...
def main():
    ...
    for factor in epst_factors:
        print(f"\n=== Solving model with rejection penalty factor {factor:.3g} ===")

        # Scale time gap for this run
        new_epsilon_t = factor * epsilon_t

        # Build model
        m, vars, bigm = build_model(params, a, c, f, HW, HL, Buffer, epsilon_t=new_epsilon_t, epsilon_p=epsilon_p)
